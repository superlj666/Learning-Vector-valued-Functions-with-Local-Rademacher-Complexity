...
function model = learner_lrc_ssl(errors_validate, can_tau_S, can_tau_A, can_tau_I)
    cv_results = reshape([errors_validate{:, 1}], [ numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)]);
    ...
    fprintf('-----LRC_SSL: %.4f\t tau_I: %s\t tau_A: %s\t tau_S: %s-----\n', ...
...
function model = learner_ssl(errors_validate, can_tau_S, can_tau_A, can_tau_I)
    cv_results = reshape([errors_validate{:, 1}], [numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)]);
    cv_results(1 : numel(can_tau_S) - 1, :, :) = 1;
    cv_results(:, :, numel(can_tau_I)) = 1;
    [~, loc_best] = min(cv_results(:));
    [d2, d3, d4] = ind2sub([numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)], loc_best);
    fprintf('-----SSL: %.4f\t tau_I: %s\t tau_A: %s\t tau_S: %s-----\n', ...
    errors_validate{loc_best, 1}, num2str(can_tau_I(d4)), num2str(can_tau_A(d3)), num2str(can_tau_S(d2)));
...
function model = learner_lrc(errors_validate, can_tau_S, can_tau_A, can_tau_I)
    cv_results = reshape([errors_validate{:, 1}], [numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)]);
    cv_results(numel(can_tau_S), :, :) = 1;
    cv_results(:, :, 1 : numel(can_tau_I) - 1) = 1;
    [~, loc_best] = min(cv_results(:));
    [d2, d3, d4] = ind2sub([numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)], loc_best);
    fprintf('-----LRC: %.4f\t tau_I: %s\t tau_A: %s\t tau_S: %s-----\n', ...
    errors_validate{loc_best, 1}, num2str(can_tau_I(d4)), num2str(can_tau_A(d3)), num2str(can_tau_S(d2)));
...
function model = learner_linear(errors_validate, can_tau_S, can_tau_A, can_tau_I)
    cv_results = reshape([errors_validate{:, 1}], [numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)]);
    cv_results(1 : numel(can_tau_S) - 1, :, :) = 1;
    cv_results(:, :, 1 : numel(can_tau_I) - 1) = 1;
    [~, loc_best] = min(cv_results(:));
    loc_best = loc_best(1);
    [d2, d3, d4] = ind2sub([numel(can_tau_S), numel(can_tau_A), numel(can_tau_I)], loc_best);
    fprintf('-----Linear: %.4f\t tau_I: %s\t tau_A: %s\t tau_S: %s-----\n', ...
    errors_validate{loc_best, 1}, num2str(can_tau_I(d4)), num2str(can_tau_A(d3)), num2str(can_tau_S(d2)));
